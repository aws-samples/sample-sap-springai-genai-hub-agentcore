// nosemgrep: bosco/fc-js-cognito-identity
...
async function authenticateSimple(username, password) {
    ...
    const auth = {
        ...
        authType: 'simple',
        sessionId: username + '-' + crypto.randomUUID()  // unique per login
    };
...
async function authenticateCognito(username, password, config) {
    if (typeof AmazonCognitoIdentity === 'undefined') {
        await new Promise((resolve, reject) => {
            ...
            script.integrity = 'sha384-OcHWOnEoU00BHNx+H54oDJAC6KhxE/d9EhQCsyzRzTRa55zZItEDA994e7jC4I9M';
        ...
        cognitoUser.authenticateUser(authenticationDetails, {
            onSuccess: function(result) {
                ...
                const auth = {
                    ...
                    expiresAt: expiresAt,
                    authType: 'cognito',
                    sessionId: (payload.sub || username) + '-' + crypto.randomUUID()  // unique per login
                };
