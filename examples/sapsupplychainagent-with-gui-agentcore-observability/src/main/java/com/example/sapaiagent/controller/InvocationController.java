...
public class InvocationController {
    ...
    public Flux<String> handleUserPrompt(InvocationRequest request, AgentCoreContext agentCoreContext) {
        ...
        // AgentCore evaluations query spans by session.id — all spans within a session
        // must share the same value so evaluations can group them.
     ...
     * We read the same header here so our application-level spans (invoke_agent,
     * execute_agent_step, execute_tool) carry the same session.id value, ensuring
     * all spans in a request are correlated under one session in CloudWatch.
     *
     * Priority:
     *   1. X-Amzn-Bedrock-AgentCore-Runtime-Session-Id — the AgentCore system
     *      header, set by the SDK's runtimeSessionId or by the GUI's fetch() call
     *   2. Fallback to userId — for local testing where no AgentCore headers exist
     */
    private String extractSessionId(AgentCoreContext context, String userId) {
        ...
        // Fallback: use userId so all requests from the same user are grouped
